...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	MSG_WAR(0x3A60, "proceedSDO ", 0);//提示现在进入sdo处理程序
	whoami = SDO_UNKNOWN;
	/* Looking for the cobId in the object dictionary. */
	/* Am-I a server ? */
	offset = d->firstIndex->SDO_SVR;//第一个服务器的下标
	lastIndex = d->lastIndex->SDO_SVR;//最后一个服务器的下标
	j = 0;
	if(offset) while (offset <= lastIndex) {//从第一个服务器遍历到最后一个服务器
		if (d->objdict[offset].bSubCount <= 1) {
			MSG_ERR(0x1A61, "Subindex 1  not found at index ", 0x1200 + j);
			return 0xFF;
		}
		/* Looking for the cobid received. *///遍历所有服务器，看哪个服务器的clent->serve 的id与接收帧的id相等，如果匹配，则就是发给它的
		pCobId = (UNS32*) d->objdict[offset].pSubindex[1].pObject;
		if ( *pCobId == UNS16_LE(m->cob_id) ) {//匹配上了
			whoami = SDO_SERVER;
			MSG_WAR(0x3A62, "proceedSDO. I am server. index : ", 0x1200 + j);//打印服务器地址
	...
	if (whoami == SDO_UNKNOWN) {//如果遍历服务器，没有匹配得上的，继续遍历client
		/* Am-I client ? */
		offset = d->firstIndex->SDO_CLT;
	...
	if (whoami == SDO_UNKNOWN) {//都匹配不上，则说明不是给我们的 直接返回
	...
	if (whoami == SDO_CLIENT) {
		MSG_WAR(0x3A68, "I am CLIENT number ", CliServNbr);//打印终端编号  比如第一个终端：  I am CLIENT number 0
	}
	else {
		MSG_WAR(0x3A69, "I am SERVER number ", CliServNbr);//打印服务器编号 比如第一个服务器： I am SERVER number 0
	}
	...
	cs = 0xFF; 
	/* Special cases for block transfer : in frames with segment data cs is not spécified */ //特殊情况下cs指令没有指定，需要根据情况赋值。如上传、下载时令s=6
   	if (!err) {
			if (((whoami == SDO_SERVER) && (d->transfers[line].state == SDO_BLOCK_DOWNLOAD_IN_PROGRESS)) || //我是服务器，现在在下载（client ->server）
				((whoami == SDO_CLIENT) && (d->transfers[line].state == SDO_BLOCK_UPLOAD_IN_PROGRESS))) {		    //我是终端，现在在上传（server-> client）
			if(m->data[0] == 0x80)	/* If first byte is 0x80 it is an abort frame (seqno = 0 not allowed) *///异常响应 cs=4
				cs = 4;
			else
				cs = 6;
		}
	}
	/* Other cases : cs is specified */ //当data[0]的数据就是cs指令时，直接提取即可
	if (cs == 0xFF)
		cs = getSDOcs(m->data[0]);//数据右移5位。此时快速sdo的cs指令：写指令 0x2F/0x2B/0x27/0x23 右移5位为1 ；读指令 0x40/0x4F/0x4B/0x47/0x43右移5位为2  写成功应答0x60右移后为3
	...
	switch (cs) {
		...
		case 1:
			...
			if (whoami == SDO_SERVER) {
				index = (UNS16)getSDOindex(m->data[1],m->data[2]);//获取地址下标
				...
				if (getSDOe(m->data[0])) { /* If SDO expedited */ //快速sdo 写数据到字典里去
					/* nb of data to be downloaded */
					nbBytes = 4 - getSDOn2(m->data[0]);
					/* Storing the data in the line structure. */
					d->transfers[line].count = nbBytes;
					err = SDOtoLine(d, line, nbBytes, (*m).data + 4);//写入数据
					...
					MSG_WAR(0x3A83, "SDO Initiate Download is an expedited transfer. Finished. ", 0);//提示 快速sdo下载完成（写入本节点字典）
					/* Transfering line data to object dictionary. */
					errorCode = SDOlineToObjdict(d, line);
					if (errorCode) {
				...
				else {/* So, if it is not an expedited transfer */ //所以，如果不是快速sdo
					if (getSDOs(m->data[0])) {
				...
				for (i = 4 ; i < 8 ; i++)
					data[i] = 0;
				sendSDO(d, whoami, CliServNbr, data);//使用服务器反馈数据（下载相应帧）
			} /* end if I am SERVER */
			else {
				/* I am CLIENT */
		...
		case 2:
			/* I am SERVER */
		/* Receive of an initiate upload.*/  //收到一个上传请求 即终端告诉服务器，该往终端里上传数据了
			if (whoami == SDO_SERVER) {
				...
				CliServNbr);//使用哪个服务器
				MSG_WAR(0x3A90, "Reading at index : ", index);//读取哪里的地址下标
				...
				/* No line on use. Great !*/
				/* Try to open a new line.*/
				err = getSDOfreeLine( d, whoami, &line );//打开新的传输通道
				if (err) {
					MSG_ERR(0x1A71, "SDO error : No line free, too many SDO in progress. Aborted.", 0);
					failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
					return 0xFF;
				}
				initSDOline(d, line, CliServNbr, index, subIndex, SDO_UPLOAD_IN_PROGRESS);//初始化一个sdo时钟进程
				...
				/* Preparing the response.*/  //获取数据，写入数据，并上传
				getSDOlineRestBytes(d, line, &nbBytes);	/* Nb bytes to transfer ? */
				if (nbBytes > 4) {//大于4个字节，不能使用快速sdo，非常麻烦
					...
					MSG_WAR(0x3A96, "SDO. Sending expedited upload initiate response defined at index 0x1200 + ",
							CliServNbr);
					sendSDO(d, whoami, CliServNbr, data);//发送
					/* Release the line.*/
					resetSDOline(d, line);
				}
			} /* end if I am SERVER*/
