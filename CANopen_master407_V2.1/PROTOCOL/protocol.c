...
extern unsigned char SoftwareError;
...
{
  static unsigned char buffer3[10];
  ...
  else if(count3 == 5)
  {
    if(rec == 0xAA)
    {
      Voltage = buffer3[0] + ((u16)buffer3[1]<<8);
      Current = buffer3[2] + ((u16)buffer3[3]<<8);
    }
    count3 = 0;
  }
  else
